...
Starter 共 38 个 Maven 模块。每个模块的总开关、嵌套属性、默认值、条件装配、可选值和生产注意均已从源码展开到[Starter 全量配置参考](/guide/config/starter)，当前覆盖 357 个条目。接入前建议按模块逐项检查，尤其关注安全、租户、签名、存储、消息、任务（XXL-JOB）、License 和 AI 的必填条件。
