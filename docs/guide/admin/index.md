...
开发种子账号为 `admin/pt5aQ5E6t8dkVkMp`（实际种子以 `V2__data.sql` 为准，注释标明"密码统一 pt5aQ5E6t8dkVkMp（与前端演示登录一致）"），只用于本地初始化。任何共享、测试或生产环境都必须在首次部署前修改或禁用该账号。演示 AK/SK 同样不得用于生产。
