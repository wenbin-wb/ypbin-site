...
- **单独上传**：`scp -r apps/web-antd/dist/* root@<服务器IP>:/opt/ypbin/main/ypbin-admin/admin-ui-dist/`（nginx bind 挂载直接读新文件无需重启；**上传后需目录权限 755/文件 644**，否则 nginx 读不到会报 js `text/html` MIME 错误）
...
登录页与前端源码**不内置任何演示口令**（不再预填默认账号）。初始账号与口令由后端部署种子/初始化流程提供（见 [Admin 部署](/guide/admin/deployment)），仅用于本地开发与联调；任何共享、测试或生产环境都必须在首次部署后立即修改或禁用。文档中的演示 AK/SK 同样仅用于本地开发，不得用于生产环境。
