...
const shortType = (type) => {
  ...
  return last ? last.charAt(0).toLowerCase() + last.slice(1) : raw
}
...
let starterBody = `---\ntitle: Starter 配置参考\ndescription: ypbin-starter 全量配置项、默认值、启用条件与生产注意。\n---\n\n# Starter 配置参考\n\n本页由源码审计数据生成，覆盖 **${starter.summary.totalEntryCount}** 个配置项，其中 \`ypbin.*\` ${starter.summary.customPropertyCount} 项、宿主标准配置 ${starter.summary.standardHostPropertyCount} 项。默认值以源码为准。表格支持左右滚动。\n`
